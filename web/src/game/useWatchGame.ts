...
export function useWatchGame(columns: number): WatchGame {
  ...
  return {
    ...
    alive,
    collapse,
    total: state?.ruleset ? rulesetSize(state) : 0,
    error,
    busy,
    answer,
    restart,
...
function rulesetSize(state: GameState): number {
  const { length, alphabet, allow_repeats } = state.ruleset;
  const symbols = alphabet.length;
  let size = 1;
  for (let taken = 0; taken < length; taken += 1) {
    size *= allow_repeats ? symbols : symbols - taken;
  }
  return size;
}
